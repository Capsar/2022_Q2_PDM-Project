...
from local_path_planning import follow_path, path_smoother,interpolate_path, pid_follow_path, velocities, angles, nodes
from urdf_env_helpers import add_obstacles, add_goal, add_graph_to_env, draw_path


def run_albert(n_steps=500000, render=True, goal=True, obstacles=False):
    robots = [
    ...
    shortest_path_configs = [robot_pos_config, [-5, -5, 0], [-5, 0, 0]]
    ...
    for step in range(n_steps):
        if not history:
            ...
            action = pid_follow_path(ob, history[-1], shortest_path_configs)
            if action == "DONE":
                break
    ...
    print("PLOTTING")
    plt.figure()
    x = range(len(velocities["velocity"]))
    plt.plot(x, velocities["velocity"], label="velocity")
    plt.plot(x, velocities["control"], label="control")
    plt.plot(x, velocities["goal"], label="goal")
    plt.plot(x, velocities["diff"], label="diff")
    for node in nodes:
        plt.axvline(x=node, color="black")
    plt.legend()
    plt.title("Velocity plots")

    plt.figure()
    plt.plot(x, angles["diff"], label="Angle difference")
    for node in nodes:
        plt.axvline(x=node, color="black")
    plt.legend()
